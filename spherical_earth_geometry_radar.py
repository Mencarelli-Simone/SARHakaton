...
def gcs_to_lcs(x, y, z, Bc2s, S0):
    """
    numpy native implementation
    :param x: 1d or 2d gcs x coordinates
    :param y: 1d or 2d gcs y coordinates
    :param z: 1d or 2d gcs z coordinates
    :param Bc2s: coordinate transformation matrix
    :param S0: origin of the local coordinate system
    :return: xl, yl, zl in lcs
    """
    sp = x.shape
    x = x.reshape(-1)
    y = y.reshape(-1)
    z = z.reshape(-1)
    # stack x y z vertically
    P = np.vstack((x, y, z))
    # remove s0
    P = P - S0.reshape(3, 1)
    # change to local
...
def mesh_doppler_to_azimuth(theta_mesh, doppler_mesh, lambda_c, v_s, h=500e3, re=6371e3, c=299792458.0):
    ...
    :return: incidence angle - azimuth mesh - Integration time
    """
    ...
    arg = ((lambda_c ** 2 * doppler_mesh ** 2 + np.sqrt(
        -4 * A * lambda_c ** 2 * v ** 2 * doppler_mesh ** 2 + 4 * B ** 2 * v ** 4 + lambda_c ** 4 * doppler_mesh ** 4)) / (
                   2 * B * v ** 2))
    # nan to 0 for full space mapping
    arg = np.where(np.isnan(arg), 0, arg)
    ...
    # azimuth coordinate
    # print(cos_theta_e)
    Az_mesh = re * v * t_k * cos_theta_e  # ground azimuth distance
...
def mesh_incidence_azimuth_to_gcs(incidence_mesh, azimuth_mesh, lambda_c=0, v_s=0, h=500e3, re=6371e3, c=299792458.0):
    """
    ...
    # limit the cosine to avoid nan
    cos_theta_e = np.where(np.isnan(cos_theta_e), 0, cos_theta_e)
    ...
    # thetae 0 if nan
    theta_e = np.where(np.isnan(theta_e), 0, theta_e)
...
def incidence_azimuth_to_doppler(incidence_angle, az, lambda_c, v_s, h=500e3, re=6371e3, c=299792458.0):
    """
    converts range and azimuth to doppler shift in spherical earth
    :param incidence_angle: incidence angle coordinate
    ...
    B = 2 * (re + h) * re * cos_theta_e
    v = v_s / (re + h)
    doppler = (-B * v * sin(v * t) / (lambda_c * np.sqrt(A - B * cos(v * t))))
    # sign retrieval
    # doppler = np.where(az < 0, doppler, -doppler)
    return doppler
...
def looking_angle_to_incidence_angle(looking_angle, h=500e3, re=6371e3):
    """
    :param looking_angle: looking angle radians
    :param h: optional, satellite height from ground, default 500 km
    :param re: optional, spherical earth radius, default 6371 km
    :return:
    """
    # Calculate r0 using the rule of cosines
    r0 = (re + h) * np.cos(looking_angle) - np.sqrt((re + h) ** 2 * np.cos(looking_angle) ** 2 - h ** 2 - 2 * h * re)

    # Calculate the cosine of the internal angle (complementary to incidence)
    cos_gamma_i = (r0 ** 2 + re ** 2 - (re + h) ** 2) / (2 * r0 * re)
    theta_i = np.pi - np.arccos(cos_gamma_i)
